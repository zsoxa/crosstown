...
public class GameController : MonoBehaviour {
	...
	public void determineCrossroads(List<FieldDescriber> world){
		...
		foreach(FieldDescriber f in world){
			if(f.getType() == 1 || f.getType() == 4 || f.getType() == 5 || f.getType() == 6 || f.getType() == 7){
				...
				Vector3 spawnPosition = new Vector3((float)fieldSize*f.getIndex().x,0.0f,(float)fieldSize*f.getIndex().y);
				Crossroad cross = (Crossroad) Instantiate(c, spawnPosition,Quaternion.AngleAxis(0.0f, Vector3.up));
	...
	protected void addNewCar(){
		CarHandler car = (CarHandler) Instantiate(car1,new Vector3(0,0,0),Quaternion.AngleAxis(0.0f, Vector3.up));
		car.transform.localScale = new Vector3(0.6f, 0.6f, 0.6f);
		carList.Add(car);
	}
	
	public void addCars(){
		for(int i=0; i<carsNumber; i++){addNewCar();}
